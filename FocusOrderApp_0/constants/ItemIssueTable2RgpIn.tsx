...
const ItemIssueTable: React.FC<ItemIssueTableProps> = ({
  ...
  //           {
  //             Query:
  //               'select top 15 sName [label], iMasterId [value] from mCore_Product where iStatus <> 5 and iMasterId <> 0 and bGroup = 0',
  //           },
  //         ],
  //       },
  //     );
  ...
  // }, [gridDataresponse]);
  React.useEffect(() => {
    async function getPendingRgp() {
      var storedHostname = await AsyncStorage.getItem('hostname');
      const rgpOutPendingResponse = await fetchDataFromApi(
        `http://${storedHostname}/focus8API/utility/executesqlquery`,
        {
          data: [
            {
              Query: `select 
  distinct  h.sVoucherNo,d.iTransactionId,eh.VendorName [VendorNameId], a.sName [VendorName],iProduct, p.sName [Item],abs(fQuantity) [Quantity]
  ...
  join tCore_HeaderData7960_0 eh on eh.iHeaderId = d.iHeaderId
  join mCore_Account a on a.iMasterId = eh.VendorName
  join tCore_Data7960_0 dd on dd.iBodyId=d.iBodyId
  join tCore_Data_Tags_0 td on td.iBodyId=d.iBodyId
  join vCore_Links521674521_0 vref on vref.iRefId=d.iBodyId
  where iVoucherType = 7960 and vref.iStatus <> 2 and h.sVoucherNo = '${selectedRgpOut?.label}'`,
            },
          ],
        },
      );
      if (
        rgpOutPendingResponse &&
        rgpOutPendingResponse?.data &&
        rgpOutPendingResponse?.result === 1 &&
        rgpOutPendingResponse?.data?.[0]?.Table &&
        rgpOutPendingResponse?.data?.[0]?.Table?.length > 0
      ) {
        setSelectedValues(rgpOutPendingResponse?.data?.[0]?.Table);
      } else {
        setSelectedValues(gridDataresponse);
      }
    }
    getPendingRgp();
    // if (selectedRgpOut?.label) {
    // }

    // setSelectedValues(gridDataresponse);
  }, [clearTable, gridDataresponse, selectedRgpOut?.label]);
  ...
  const handleCheckBoxChange = (row: any) => {
    ...
    updatedSelectedValues[row] = {
      ...
      ItemId: updatedSelectedValues[row]?.iProduct,
      VendorName: updatedSelectedValues[row]?.VendorName,
      iTransactionId: updatedSelectedValues[row]?.iTransactionId,
      VendorNameId: updatedSelectedValues[row]?.VendorNameId,
    ...
    onData({filteredItems: filteredSelectedValues});
    setSelectedValues(updatedSelectedValues);
    // setselectedCompany(date.label);
  };
  ...
  return (
    <ScrollView horizontal={true} style={{flex: 1}}>
      ...
      {!selectedValues?.[0]?.isCheckBoxDisable && (
            ...
            {selectedValues?.map((item: any, row: number) => (
              <View
                ...
                style={[
                  ...
                  selectedValues?.[row]?.isRowSelected
                    ? styles?.selectedRow
                    : null,
