...
import {createExpenseAccTable, insertExpenseAcc} from './OrdersServices';
...
export const syncExpenseAccounts = async () => {
  try {
    const storedHostname = await AsyncStorage.getItem('hostname');
    const storedFocusSession = await AsyncStorage.getItem('focusSessoin');
    var storedPOSSalePreferenceData: any = await AsyncStorage.getItem(
      ...
      `${storedHostname}/focus8API/utility/executesqlquery`,
      {
        ...
        body: JSON.stringify({
          data: [
            {
              Query: `
select a.iMasterId value, a.sName label from mCore_Account a
join mCore_AccountTreeDetails at on at.iMasterId = a.iMasterId
where at.iParentId = 26304 and a.iStatus <>5 and a.iMasterId <> 0;`,
            },
          ],
        }),
        signal: controller.signal, // Attach the abort signal
    ...
    if (data.result === 1 && data.data?.[0]?.Table) {
      const db = await getDBConnection();
      await createExpenseAccTable(); // Create the ExpenseAcc table
      await insertExpenseAcc(db, data.data[0].Table); // Insert the ExpenseAcc data

      return {
        success: true,
        message: `Successfully synced ${data.data[0].Table.length} Expense Accounts`,
        data: data.data[0].Table,
      };
    } else {
      throw new Error(data.message || 'Failed to sync Expense Accounts');
    }
  } catch (error: any) {
    console.error('Error syncing Expense Accounts:', error);
    return {
      success: false,
      message: error.message || 'Failed to sync Expense Accounts',
      error,
    };
  }
};
