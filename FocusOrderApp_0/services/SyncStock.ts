...
export const syncStock = async () => {
  try {
    ...
    console.log("storedPOSSalePreferenceData",parsedPOSSalesPreferences?.warehouseId);
    ...
    const response = await fetch(`${storedHostname}/focus8API/utility/executesqlquery`, {
      ...
      body: JSON.stringify({
        data: [{
          Query: `select sBatchNo, iBatchId,iExpiryDate, iExpiryDateId,sum(Batch_Qty) BatchQty, iProduct,iInvTag from (
