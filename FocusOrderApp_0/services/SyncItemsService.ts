        ...
        `${storedHostname}/focus8API/utility/executesqlquery`,
        {
          ...
          body: JSON.stringify({
            ...
            mp.pImageName,
                  ...
                  JOIN muCore_Product_Units u ON u.iMasterId = p.iMasterId
				  JOIN mCore_itemtype mc on mc.iMasterId = mp.ItemType
                  WHERE p.iStatus = 0 AND p.bGroup = 0 AND p.iMasterId <> 0;
                  select  pr.iRowIndex, pr.iMasterId, pr.iInvTag, pr.fInvReorderLevel from mCore_Product p
join muCore_Product_Reorder_Details pr on pr.iMasterId = p.iMasterId
where p.iStatus = 0 AND p.bGroup = 0 AND p.iMasterId <> 0;`,
              },
            ],
          }),
          ...
          (tx: {executeSql: (arg0: string, arg1: any[]) => void}) => {
            productData.data[0].Table.forEach(
              (product: {
                ...
                ProductCode: any;
                Rate: any;
                iDefaultBaseUnit: any;
              }) => {
                tx.executeSql(
                  `INSERT OR REPLACE INTO Products (
                ProductId, CategoryId, CategoryName, ProductName, ProductCode, Rate, CurrencyId, CurrencyCode, iDefaultBaseUnit
              ) VALUES (?, ?, ?, ?, ?, ?,?,?,?);`,
                  [
                    product.ProductId,
                    product.CategoryId,
      ...
      if (
        ...
        productData.data?.[0]?.Table1?.length > 0
      ) {
        await insertReorder(db, productData.data?.[0]?.Table1);
      }
      ...
      `${storedHostname}/focus8API/utility/executesqlquery`,
      {
        ...
        body: JSON.stringify({
          data: [
            {
              Query: `SELECT  CASE 
    WHEN Image IS NULL OR LEN(Image) = 0 THEN ''
    ELSE 
      CASE
        WHEN CAST(N'' AS XML).value('xs:base64Binary(xs:hexBinary(sql:column("Image")))', 'NVARCHAR(MAX)') = 'AA==' THEN ''
        ELSE CAST(N'' AS XML).value('xs:base64Binary(xs:hexBinary(sql:column("Image")))', 'NVARCHAR(MAX)')
      END
