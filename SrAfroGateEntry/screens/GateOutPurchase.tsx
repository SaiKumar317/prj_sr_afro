...
import GateOutPurchasePage from '../pages/GateOutPurchasePage';
...
function GateOutPurchase({
  ...
  const handleGateInPurchasePage = (data: React.SetStateAction<any>) => {
    ...
    console.log('setGateOutPurchasePage', data);
  ...
  const handleSaveButtonPressed = async () => {
    try {
      setIsLoading(true);
      console.log('Data from GateOutPurchasePage:', gateInPurchasePage);
      ...
      if (
        ...
        if (storedHostname) {
          ...
          if (
            ...
            const bodyData = [];
            for (let x in gateInPurchasePage?.gridDataresponse) {
              bodyData.push({
                Item__Id: gateInPurchasePage?.gridDataresponse[x]?.iProduct,
                Unit__Id: gateInPurchasePage?.gridDataresponse[x]?.iUnit,
                Quantity: gateInPurchasePage?.gridDataresponse[x]?.qty,
                'L-Gate Entry In':
                  gateInPurchasePage?.gridDataresponse[x]?.iTransactionId,
              });
            }
              ...
              data: [
                {
                  ...
                  Header: {
                    ...
                    // Department__Id: selectedCompBranch?.companyId,
                    Segment__Id: selectedCompBranch?.companyId,
                    // Company__Id: selectedCompBranch?.companyId,
                    'Sub Division__Id':
                      gateInPurchasePage?.gridDataresponse[0]?.iTag3001,
                    // Division__Id:
                    //   gateInPurchasePage?.gridDataresponse[0]?.iTag3001,
                    ...
                    Image1: {
                      FileName: `purchase_out${dateToInt(new Date())}_${
                        getIntDate?.data?.[0]?.Table?.[0]?.Time
                      }.jpg`,
        ...
        console.error('Invalid gateOutPurchasePage:', gateInPurchasePage);
        ...
        if (postingFailedMsg.length === 0) {
          ...
          //   alert(`No MRN available for the ${selectedGateIn?.label}`);
          // }
          if (gateInPurchasePage?.gridDataresponse?.length === 0) {
...
export default GateOutPurchase;
