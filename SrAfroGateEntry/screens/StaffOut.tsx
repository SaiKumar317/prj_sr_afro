...
import StaffOutPage from '../pages/StaffOutPage';
...
function StaffOut({
  ...
  const handleGateInPurchasePage = (data: React.SetStateAction<any>) => {
    ...
    console.log('setStaffOutPage', data);
  ...
  const handleSaveButtonPressed = async () => {
    try {
      setIsLoading(true);
      console.log('Data from StaffOutPage:', gateInPurchasePage);
      ...
      console.log(
        ...
        gateInPurchasePage &&
          ...
          gateInPurchasePage?.selectedDepartmentName &&
      ...
      if (
        ...
        typeof gateInPurchasePage === 'object' &&
        gateInPurchasePage?.selectedDepartmentName &&
        gateInPurchasePage?.toPlace &&
        gateInPurchasePage?.purpose &&
        gateInPurchasePage?.capturedImage1 &&
        ...
        if (storedHostname) {
          ...
          if (
            ...
            gateEntryInUrl = `${storedHostname}/focus8api/Transactions/7985/`;
            const bodyData = [{Quantity: 1}];
              ...
              data: [
                {
                  ...
                  Header: {
                    ...
                    'Department Master__Id': selectedDepartmentName?.value,
                    VehicleNo: vehicleNo || '',
                    ToPlace: toPlace || '',
                    Purpose: purpose || '',
                    OutTime: getIntDate?.data[0]?.Table[0]?.Time,
                    Image1: {
                      FileName: `staff_out${dateToInt(new Date())}_${
                        getIntDate?.data[0]?.Table[0]?.Time
                      }.jpg`,
        ...
        console.error('Invalid StaffOutPage:', gateInPurchasePage);
        ...
        if (!selectedDepartmentName) {
          postingFailedMsg.push('Department Name');
        }
        if (!toPlace) {
          postingFailedMsg.push('To Place');
        }
        if (!purpose) {
          postingFailedMsg.push('Purpose');
        }
