...
import StaffInPage from '../pages/StaffInPage';
...
function StaffIn({
  ...
  const handleGateInPurchasePage = (data: React.SetStateAction<any>) => {
    ...
    console.log('setStaffInPage', data);
  ...
  const handleSaveButtonPressed = async () => {
    try {
      setIsLoading(true);
      console.log('Data from StaffInPage:', gateInPurchasePage);
      ...
      console.log(
        ...
        gateInPurchasePage &&
          ...
          gateInPurchasePage?.selectedStaffOut &&
          gateInPurchasePage?.employeeName,
      );
      if (
        gateInPurchasePage &&
        gateInPurchasePage !== null &&
        ...
        gateInPurchasePage?.selectedStaffOut &&
        gateInPurchasePage?.capturedImage1 &&
        gateInPurchasePage?.employeeName?.employeeCode
      ) {
        const {
          capturedImage1,
          selectedCompBranch,
          ...
          const getPendingVoucherData = await fetchDataFromApi(
            `${storedHostname}/focus8API/utility/executesqlquery`,
            {
              data: [
                {
                  Query: `select 
distinct  h.sVoucherNo,d.iTransactionId,eh.EmpID ,emp.sName [Employee], emp.iMasterId [empMasterId], dept.sName [department],dept.iMasterId [deptId], eh.VehicleNo, cb.sName [compBranch],eh.ToPlace,
eh.Purpose, eh.OutTime 
...
where iVoucherType = 7985 and vref.iStatus <> 2 and h.sVoucherNo = '${selectedStaffOut?.sVoucherNo}'`,
                },
              ],
            },
          );
          ...
          if (
            ...
            const staffOutPending = getPendingVoucherData?.data?.[0]?.Table;
            gateEntryInUrl = `${storedHostname}/focus8api/Transactions/7986/`;
            const bodyData = [];
            for (let x in staffOutPending) {
              bodyData.push({
                Quantity: 1,
                'L-Staff Movement Out': staffOutPending?.[x]?.iTransactionId,
              });
            }
              ...
              data: [
                {
                  ...
                  Header: {
                    ...
                    Employee__Id: staffOutPending?.[0]?.empMasterId,
                    'Department Master__Id': staffOutPending?.[0]?.deptId,
                    VehicleNo: staffOutPending?.[0]?.VehicleNo || '',
                    // ToPlace: selectedStaffOut?.ToPlace || '',
                    // Purpose: selectedStaffOut?.Purpose || '',
                    OutTime: staffOutPending?.[0]?.OutTime,
                    InTime: getIntDate?.data[0]?.Table[0]?.Time,
                    Image1: {
                      FileName: `staff_in${dateToInt(new Date())}_${
                        getIntDate?.data[0]?.Table[0]?.Time
                      }.jpg`,
        ...
        console.error('Invalid StaffInPage:', gateInPurchasePage);
        const postingFailedMsg = [];
        if (!employeeName) {
          postingFailedMsg.push('Employee Name');
        }

        if (!selectedStaffOut) {
          postingFailedMsg.push('Staff Out Doc No');
        }
