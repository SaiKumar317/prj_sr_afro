...
import RGPOutPage from '../pages/RGPOutPage';
...
function ReturnableGPOut({
  ...
  const handleGateInPurchasePage = (data: React.SetStateAction<any>) => {
    ...
    console.log('setRGPOutPage', data);
  ...
  const handleSaveButtonPressed = async () => {
    try {
      setIsLoading(true);
      console.log('Data from RGPOutPage:', gateInPurchasePage);
      ...
      if (
        ...
        if (storedHostname) {
          ...
          if (
            ...
            gateEntryInUrl = `${storedHostname}/focus8api/Transactions/7960/`;
            const bodyData = [];
              ...
              data: [
                {
                  ...
                  Header: {
                    ...
                    'Sub Division__Id': selectedDivision?.iMasterId,
                    // Division__Id: selectedDivision?.iMasterId,
                    ...
                    OutTime: getIntDate?.data[0]?.Table[0]?.Time,
                    ...(estimateDate && {
                      ExpectedReturn: dateToInt(new Date(estimateDate)),
                    }),
                    Employee__Code: `${employeeName?.employeeCode}`,
                    VendorName__Id: selectedVendorName?.value,
                    VehicleNo: vehicleNo || '',
                    ...
                    Image1: {
                      FileName: `rgp_out${dateToInt(new Date())}_${
                        getIntDate?.data[0]?.Table[0]?.Time
                      }.jpg`,
        ...
        console.error('Invalid RGPOutPage:', gateInPurchasePage);
        const postingFailedMsg = [];
        if (!selectedEntryTypeName) {
          postingFailedMsg.push('Type');
        }
        if (!toPlace) {
          postingFailedMsg.push('To Place');
        }
        if (!selectedVendorName) {
          postingFailedMsg.push('Vendor Name');
        }
        if (!selectedDivision) {
          postingFailedMsg.push('Division');
        }
        if (!employeeName?.employeeCode) {
          postingFailedMsg.push('Responsible Person');
        }
        // if (!vendorContact) {
        //   postingFailedMsg.push('Vendor Contact No');
        // }
        // if (!estimateDays) {
        //   postingFailedMsg.push('Estimate Days');
        // }
        // if (!vehicleNo) {
...
export default ReturnableGPOut;
