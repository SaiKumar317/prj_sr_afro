...
import ItemIssuseTableSaleIn from '../constants/ItemIssuseTableSaleIn';
...
const GateInSalePage: React.FC<GateInPurchasePageProps> = ({
  onData,
  masterResponse,
  // gridDataresponse,
  reloadPage,
  ...
  const handleSelectedPendingVochNo = async (data: any) => {
    ...
    if (data && data?.length > 0 && pendingVochNo) {
      ...
      const updatedGridDataresponse = gridDataresponse?.map(
        (item: {Balance: any}) => ({
          ...item,
          invoiceQty: `${item?.Balance}`,
        }),
      );
      setGridDataresponse(updatedGridDataresponse);
      setSelectedGridData([]);
    }
    onData({
      selectedVoucher,
